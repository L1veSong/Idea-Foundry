...
**全局调度中枢 · role: orchestrator · 通用开源级自组装工作流引擎**
一个想法 → 一键锻造 → 可交付成果
---
## 模式
### 三种策略
...
### 全局调度（角色声明制）
```
role: orchestrator  Idea Foundry         ← 最先触发, 全局调度中枢
role: fallback      Superpowers          ← 降级为能力 Skill
role: fallback      GStack autoplan      ← 降级为能力 Skill
role: last_resort   外部原生工作流         ← 仅备选
role: ultimate_fallback 人工引导          ← 最后兜底
```
### 质量预检
```
...
- 🧹 **数字优先级彻底移除**: frontmatter + JSON 配置全部清理
- 📋 **global-priority.json**: `level` 字段删除，仅保留 `role`
- 📝 **SKILL.md**: Lv.110/100/80/... → 角色声明制描述
- 🔧 **调度冲突解决表**: 数字等级 → 角色标签
